...
def mseed_predictor(input_dir='downloads_mseeds',
    ...
    # print('============================================================================')
    eqt_logger = logging.getLogger("VIT")
    eqt_logger.info(f"Running VIT")
    ...
    features = create_vit_classifier(inputs)
    features = Reshape((6000,1))(features)

    #features1  = Conv1D(1,  11, strides =(1), padding='same',kernel_initializer='he_normal')(features)
    #features1 = BatchNormalization()(features1)    
    #features1 = Activation('relu')(features1)

    logits  = Conv1D(1,  15, strides =(1), padding='same',activation='sigmoid', kernel_initializer='he_normal',name='picker_P')(features)
    ...
    for ct, st in enumerate(station_list):
        ...
        for _, month in enumerate(uni_list):
            ...
            predP = model.predict_generator(pred_generator)
            ...
            for ix in range(len(predP)):
                ...
                detection_memory,prob_memory=_output_writter_prediction(meta, csvPr_gen, Ppicks, Pprob, detection_memory,prob_memory,predict_writer, ix,len(predP))
...
def _output_writter_prediction(meta, csvPr, Ppicks, Pprob, detection_memory,prob_memory,predict_writer, idx, cq):
